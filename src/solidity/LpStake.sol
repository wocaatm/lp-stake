...
contract LpStake is Ownable, ReentrancyGuard {
    address public immutable mamiStakeAddress;
    ...
    function setPrecent (uint256 precent) external onlyOwner {
        require(precent <= 100, "precent overflow");
        userRewardsPrecent = precent;
    }
    ...
    function stake(
        ...
        // todo - referaddress
        MamiStakeV2(mamiStakeAddress).stake(_poolId, _tokenIds, _syncTokenIds);
    }
    ...
    function getRewardsAmount(uint256 _poolId, uint256[] calldata _tokenIds) public view returns(uint256, uint256) {
        ...
        uint256 diffRewards = diffTokenAmount * userRewardsPrecent / 100;
        return (totalRewardAmount - diffRewards, diffRewards);
    }
}